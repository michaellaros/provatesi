...
namespace TDTK{
	...
	public class AttackInfo{
		...
		public AttackInfo(Unit sUnit, Unit tUnit, int type, bool useAOE=true){	//type: 0-turret, 1-aoe, 2-mine
			...
			if(type==1){	//aoe
				if(Random.value>srcUnit.GetHit_AOE()-tgtUnit.GetDodge()) return;
				...
				if(Random.value>srcUnit.GetHit_Mine()-tgtUnit.GetDodge()) return;
				
				damageMin=srcUnit.GetDamageMin_Mine();
				damageMax=srcUnit.GetDamageMax_Mine();
				aoeRange=useAOE ? srcUnit.GetAOERange_Mine() : 0;
				...
				if(Random.value>srcUnit.GetHit()-tgtUnit.GetDodge()) return;
